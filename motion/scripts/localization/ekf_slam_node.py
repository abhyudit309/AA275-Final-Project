...
# Runs EKF SLAM with known correspondences
...
from utils.ekf_slam import EkfSlam
from utils.ExtractLines import ExtractLines
from utils.maze_sim_parameters import LineExtractionParams, NoiseParams, ARENA, ArenaParams
...
class EKF_SLAM_Visualizer(Node):
    def __init__(self):
        super().__init__('turtlebot_mapping')
        ...
        self.P0_map = np.diag(np.array([[NoiseParams["std_alpha"]**2 for _ in range(N_map_lines)], [NoiseParams["std_r"]**2 for _ in range(N_map_lines)]]).T.flatten())
